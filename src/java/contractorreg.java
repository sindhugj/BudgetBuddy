 ...
 * @author Sindhusha
 */
@WebServlet(name = "contractorreg", urlPatterns = {"/contractorreg"})
...
String citizen,psw,village,district,state;
          ...
          String dname=session.getAttribute("sdname").toString();
           ...
           String q="insert into contractordata values('0','"+citizen+"','"+psw+"','"+village+"','"+dname+"','"+state+"')";
           java.sql.Statement st=con.createStatement();
          int i=st.executeUpdate(q);
          if(i>0)
          {
               RequestDispatcher rd=request.getRequestDispatcher("contractorreg.jsp");
                   request.setAttribute("msg", "Contractor Officer Registered Successfully");
                   rd.forward(request, response);
          }else
          {
               RequestDispatcher rd=request.getRequestDispatcher("citizenreg.jsp");
                   request.setAttribute("msg", "Contractor Not Registered");
                   rd.forward(request, response);
          }
